...
async function checkAlreadyPurchased(client, accountId, chapterId) {
    const res = await client.query(
...
async function getBalance(client, accountId) {
    const res = await client.query(
        `SELECT "coin" FROM "account" WHERE "accountId" = $1 FOR UPDATE`,
        [accountId]
    );
...
async function transferCoins(client, buyerId, uploaderId, amount) {
    await client.query(`UPDATE "account" SET "coin" = "coin" - $1 WHERE "accountId" = $2`, [amount, buyerId]);
    await client.query(`UPDATE "account" SET "coin" = "coin" + $1 WHERE "accountId" = $2`, [amount, uploaderId]);
}

// Ghi lịch sử giao dịch và trả transactionId
async function createTransaction(client, accountId, chapterId, price) {
    const res = await client.query(
...
async function recordChapterPurchase(client, accountId, chapterId, transactionId) {
    await client.query(
